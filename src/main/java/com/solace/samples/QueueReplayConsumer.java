...
public class QueueReplayConsumer {
    ...
    public static void main(String... args) throws JCSMPException, InterruptedException {
        ...
        if (!session.isCapable(CapabilityType.MESSAGE_REPLAY)) {
            ...
            System.out.println("Aborting...since MESSAGE_REPLAY isn't enabled in the message broker");
            System.out.println();
            System.exit(-1);
        } else {
            System.out.println("MESSAGE_REPLAY is enabled in the message broker");
        ...
        // setup the replay request - from a particular date or all available
        ReplayStartLocation startLocation = null;
        String dateStr = null; // Exact date example:
        // String dateStr = "2019-04-05T13:37:00";
        if (dateStr != null) {
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
            simpleDateFormat.setTimeZone(TimeZone.getTimeZone("UTC")); // This line converts the given date into UTC
                                                                       // time zone
            Date date;
            try {
                date = simpleDateFormat.parse(dateStr);
                startLocation = JCSMPFactory.onlyInstance().createReplayStartLocationDate(date);
            } catch (ParseException e1) {
                e1.printStackTrace();
            }
        }
        else {
            startLocation = JCSMPFactory.onlyInstance().createReplayStartLocationBeginning();
        }
        flow_prop.setReplayStartLocation(startLocation);
